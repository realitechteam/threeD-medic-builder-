...
interface ViewportProps {
  ...
  onDropAsset: (type: string, subType: string | null, position: Vector3Tuple) => void;
}
...
const DropHandler = ({ onDrop }: { onDrop: (type: string, subType: string | null, pos: Vector3Tuple) => void }) => {
  const { camera, gl } = useThree();

  useEffect(() => {
    const handleDropped = (e: any) => {
      const { type, subType, clientX, clientY } = e.detail;
      ...
      if (raycaster.ray.intersectPlane(plane, targetPos)) {
        onDrop(type, subType, [targetPos.x, targetPos.y, targetPos.z]);
      } else {
        onDrop(type, subType, [0, 0, 0]);
      }
    };
