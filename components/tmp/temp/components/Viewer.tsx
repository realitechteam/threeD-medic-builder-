...
const Player = ({ initialPos, onMoveUpdate }: { initialPos: Vector3Tuple, onMoveUpdate: (pos: THREE.Vector3) => void }) => {
  ...
  useFrame((state, delta) => {
    ...
    // We keep height based on the spawn point's Y + standard eye height
    // But allow some slight deviation if there was floor detection (not implemented here for simplicity)
...
const Viewer: React.FC<ViewerProps> = ({ project, onExit }) => {
  ...
  const handleInteraction = (scene: THREE.Scene, camera: THREE.Camera) => {
    ...
    if (intersects.length > 0) {
      ...
      if (currentStep?.targetAction === 'click' && assetId === currentStep.targetAssetId) {
         handleNext();
      } else if (currentStep?.targetAction === 'move' && assetId === currentStep.targetAssetId && !isHolding) {
         setIsHolding(true);
      }
    }
  };
